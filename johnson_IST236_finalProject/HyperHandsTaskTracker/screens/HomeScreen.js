  ...
  util.useFocusEffect(
    util.useCallback(() => {
      const fetchProjectsData = async () => {
        ...
        const tasksCompletedPercentage = projects.length > 0 ? (completedProjects.length / projects.length) * 100 : 0;
      ...
      <util.View style={styles.homeContent}>
        <util.View style={styles.widgets}>
          <util.View style={styles.row}>
            {/* to insert sample data, change the onPress to "insertDummyData" */}
            { /* to clear existing data, change the onPress to "clearAllData"*/}
            <util.TouchableOpacity style={styles.homeCard} onPress={openProjectsPage}>
